...
void ct_divide_impl(BigInt& q_out, BigInt& r_out, const BigInt& x, const BigInt& y) {
   ...
   const word v = reciprocal_word_ct(yn[y_words - 1]);
   ...
   if(y_words == 1) {
      ...
      */
      ...
      for(size_t i = x_words; i > 0; --i) {
         const auto [qi, ri] = bigint_div2by1_preinv(r, rw[i - 1], d, v);
         qw[i - 1] = qi;
         r = ri;
      }
      ...
      for(size_t i = q_words; i > 0; --i) {
         ...
         */
         ...
         const word q_est = bigint_div2by1_preinv(u2, u1, yn1, v).first;
...
void ct_divide_word(const BigInt& x, word y, BigInt& q_out, word& r_out) {
   ...
   for(size_t i = x_words; i > 0; --i) {
      const auto [qi, ri] = div_y.divmod_2to1(r, x._data()[i - 1]);
      qw[i - 1] = qi;
      r = ri;
   }
