   ...
   return
      [engine = std::move(engine), a = FrodoSeedA(seed_a)](std::span<uint8_t> out, uint16_t first_row, size_t nrows) {
         ...
         for(size_t r = 0; r != nrows; ++r) {
            store_le(static_cast<uint16_t>(first_row + r), &row_ids[sizeof(uint16_t) * r]);
            ids[r] = std::span(row_ids).subspan(sizeof(uint16_t) * r, sizeof(uint16_t));
            seeds[r] = std::span<const uint8_t>(a.get());
